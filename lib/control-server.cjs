...
function createControlServer(opts) {
  ...
  function buildPreflight() {
    ...
    checks.push(undeclared.length === 0 ? {
      ...
      title: '환경 변수 선언 OK',
      detail: `모든 코드 ENV 변수가 .env*에 선언됨`,
    } : {
      key: 'env-undeclared', status: 'fail',
      title: `미선언 환경 변수 ${undeclared.length}개`,
      detail: `배포 시 undefined로 동작. AI에게 .env.example 추가 요청.`,
    ...
    checks.push(leaks.varCount === 0 ? {
      ...
      title: 'frontend에 server-only env 노출 없음',
    } : {
      key: 'secret-leak', status: 'fail',
      title: `frontend 코드에 server-only env 변수 ${leaks.varCount}개 노출`,
      detail: `public prefix(NEXT_PUBLIC_/VITE_ 등) 없는 변수가 브라우저 번들에 포함됨. 키 유출 위험.`,
    ...
    checks.push(httpUrls.length === 0 ? {
      ...
      title: '외부 호출 모두 HTTPS',
    } : {
      key: 'http-urls', status: 'fail',
      title: `평문 http:// 외부 호출 ${httpUrls.length}개`,
      detail: `중간자 공격 가능. https 또는 환경 변수로 도메인 분리.`,
    ...
    checks.push(uncoveredHubs.length === 0 ? {
      ...
      title: '주요 hub 파일 모두 테스트로 보호됨',
    } : {
      key: 'hub-tests', status: 'warn',
      title: `테스트 없는 hub ${uncoveredHubs.length}개`,
      detail: `mass≥10 파일인데 테스트 없음. 배포 후 회귀 위험.`,
    ...
    checks.push(orphanRatio < 0.3 ? {
      ...
      title: `고립 파일 ${Math.round(orphanRatio * 100)}% — 정상 범위`,
    } : {
      key: 'orphans', status: 'warn',
      title: `고립 파일 비율 ${Math.round(orphanRatio * 100)}% (${orphans.length}/${files.length})`,
      detail: `30% 초과. 죽은 코드 가능성 — \`fg3d legacy\` 로 정리.`,
    })
    ...
    if (dynRatio > 0.1) {
      checks.push({
        ...
        title: `동적 import 비중 ${Math.round(dynRatio * 100)}%`,
        detail: `정적 분석 불완전. blast/bundle 결과에 caveat 있음.`,
      })
    }
    ...
    if (unused.length > 0) {
      checks.push({
        ...
        title: `사용 안 되는 env 변수 ${unused.length}개`,
        detail: `.env* 정리 후보. 옛 키 노출 가능.`,
  ...
  function buildSuggestions(topN = 10) {
    const suggestions = []
    ...
    if (undeclared.length > 0) {
      suggestions.push({
        ...
        advice: `AI에게: ".env.example에 다음 변수들의 placeholder 추가해줘 — ${undeclared.slice(0, 5).map((v) => v.var).join(', ')}${undeclared.length > 5 ? ' …' : ''}"`,
    ...
    if (uncoveredHubs.length > 0) {
      ...
      suggestions.push({
        priority: 'high',
        title: `테스트 없는 허브 파일 ${uncoveredHubs.length}개`,
        why: `많은 파일이 import하는 핵심인데 테스트로 보호 안 됨. AI 수정시 다른 곳 깨질 위험.`,
        advice: `AI에게: "${worst[0].id} 에 대한 unit test 작성해줘 — ${worst[0].mass}개 파일이 이걸 import함"`,
    ...
    if (orphans.length >= 10) {
      suggestions.push({
        priority: 'medium',
        title: `고립 파일 ${orphans.length}개 (의심 죽은 코드)`,
        why: `아무도 import하지 않고, 아무것도 import하지 않음. 보통 옛 실험/사용 안 되는 컴포넌트.`,
        advice: `AI에게: "${orphans[0].id} 등 다음 파일들이 실제 쓰이는지 확인하고 죽은 코드면 제거해줘"  (\`fg3d legacy\` 로 자세히)`,
    ...
    if (unused.length > 0) {
      suggestions.push({
        priority: 'medium',
        title: `사용 안 되는 환경 변수 ${unused.length}개`,
        why: `.env*에 있지만 코드에서 아무도 안 씀. 옛 키 노출 위험 + 정리 후보.`,
        advice: `AI에게: ".env에서 다음 변수들 제거 또는 사용처 확인해줘 — ${unused.slice(0, 5).map((v) => v.var).join(', ')}${unused.length > 5 ? ' …' : ''}"`,
    ...
    if (highConfVendors.length > 0) {
      suggestions.push({
        priority: 'medium',
        title: `Third-party 폴더 ${highConfVendors.length}개 graph에 포함됨`,
        why: `${highConfVendors.map((v) => v.path).slice(0, 3).join(', ')} 등이 vendored 코드로 보임. graph 분석/hub/orphan 결과가 오염될 수 있음.`,
        advice: `.fg3dignore에 추가 (한 줄씩, 끝에 /):\n  ${highConfVendors.slice(0, 5).map((v) => v.path + '/').join('\n  ')}`,
    ...
    if (dynRatio > 0.1) {
      suggestions.push({
        priority: 'low',
        title: `동적 import 패턴 비중 ${Math.round(dynRatio * 100)}%`,
        why: `정적 분석으로 의존성 추적이 어려움. blast/bundle 결과가 일부 누락됨.`,
        advice: `AI에게: "${dynamicFiles[0]?.id} 등 동적 import 사용처 정적 import로 가능한지 검토"`,
  ...
  function handleControlRequest(req, res) {
    ...
    try {
      ...
      if (req.method === 'GET' && seg0 === 'preflight' && rest.length === 0) {
        return writeJson(res, 200, withMeta(buildPreflight()))
      }
      ...
      if (req.method === 'GET' && seg0 === 'suggest' && rest.length === 0) {
        ...
        return writeJson(res, 200, withMeta(buildSuggestions(top)))
      }
