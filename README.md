...
- **Measured, not asserted** — Layer-2 call edges: **0% wrong-edge** on an independent position oracle in-repo, **0–1.3%** across a 13-language corpus (position oracle on same-file ambiguous names; the harder nested-scope **#M1** case is a separate, tracked limit — [details + scope](./docs/benchmarks.md), reproduce `node scripts/_precision_pos.mjs`). Token cost vs reading files: **~98–99.9% fewer** (naive-read baseline = upper bound; [how it's measured](./docs/benchmarks.md)), 1 MCP call instead of N file reads. The blast radius you act on is measured.
