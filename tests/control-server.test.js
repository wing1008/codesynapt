...
describe('control-server endpoints', () => {
  ...
  it('GET /blast/:id?compact caps per-hop lists but keeps the counts (2026-06-02)', async () => {
    const full = await call('GET', '/blast/src/foo.ts?depth=3&dir=users')
    const c = await call('GET', '/blast/src/foo.ts?depth=3&dir=users&compact=1&limit=1')
    expect(c.compact).toBe(true)
    expect(c.totalFiles).toBe(full.totalFiles)           // scalar summary preserved
    expect(c.tokenEstimate).toBe(full.tokenEstimate)
    for (const d of c.byDepth) {
      expect(d.ids.length).toBeLessThanOrEqual(1)        // capped to limit
      expect(d.count).toBeGreaterThanOrEqual(d.ids.length)
    }
  })
